  ...
  let expr' = map_expr
    (fun node ->
	  ...
	  Concat (Var_ref Texture, lx) ->
	    set_tex lx; default_cat_swap Texa lx
	| Concat (Var_ref Raster, lx) ->
	    set_ras lx; default_cat_swap Rasa lx
	| Select (Var_ref Texture, lx) ->
	    set_tex lx; default_swap Texa lx
	| Select (Var_ref Raster, lx) ->
	    set_ras lx; default_swap Rasa lx
	| Var_ref Texture ->
	    set_tex [| X; X; X; A |]; Var_ref Texa
	| Var_ref Raster ->
	    set_ras [| X; X; X; A |]; Var_ref Rasa
        | x -> x
      end else begin
        match node with
	  Concat (Var_ref Texture, ([| A |] | [| A; A; A |]
				    | [| A; A; A; _ |] as lx)) ->
	    set_tex [| X; X; X; A |]; default_cat_swap Texa lx
	| Concat (Var_ref Raster, ([| A |] | [| A; A; A |]
				   | [| A; A; A; _ |] as lx)) ->
	    set_ras [| X; X; X; A |]; default_cat_swap Rasa lx
	| Concat (Var_ref Texture, lx) ->
	    set_tex lx; default_cat_swap Texc lx
	| Concat (Var_ref Raster, lx) ->
	    set_ras lx; default_cat_swap Rasc lx
	| Select (Var_ref Texture, ([| A |] | [| A; A; A |]
				    | [| A; A; A; _ |] as lx)) ->
	    set_tex [| X; X; X; A |]; default_swap Texa lx
	| Select (Var_ref Raster, ([| A |] | [| A; A; A |]
				   | [| A; A; A; _ |] as lx)) ->
	    set_ras [| X; X; X; A |]; default_swap Rasa lx
	| Select (Var_ref Texture, lx) ->
	    set_tex lx; default_swap Texc lx
	| Select (Var_ref Raster, lx) ->
	    set_ras lx; default_swap Rasc lx
	| Var_ref Texture ->
	    set_tex [| R; G; B; X |]; Var_ref Texc
	| Var_ref Raster ->
	    set_ras [| R; G; B; X |]; Var_ref Rasc
	| x -> x
      end)
    expr in
...
(* Rewrite "tev.xxx" variables as cprev or aprev as appropriate to context.
   Also rewrite cr0, cr1, cr2 as c0/a0/c1/a1/c2/a2 similarly. And also
   ras/tex as Texc/Texa/Rasc/Rasa.  *)
